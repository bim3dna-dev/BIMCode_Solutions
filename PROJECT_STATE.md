...
## M3 Hobby rate-limit patch (2026-09-14)

This supersedes the earlier two-WAF-rule requirement. Owner reports the Hobby deployment permits one custom rate-limit rule. **Existing `rate-limit-audit-analysis`: POST `/api/audit/analyze`, 3 requests / 600 seconds / IP, 429 remains unchanged.** POST `/api/audit/interview` now has a mandatory distributed Redis fixed-window limiter: **18 requests / 600 seconds / IP**, using the existing M3 REST store and credentials. No new dependency or environment variable.

One atomic Lua EVAL starts expiry on the first accepted request; rejected requests neither increment nor extend the window. Independent IP digests have counters shared across Vercel instances. Limiting precedes JSON/schema processing, interview-state access and provider creation. Blocked requests return 429/Retry-After with the existing frontend message; no interview-state mutation or model call. Redis failures return generic 503, never bypass. Trust only single valid `x-vercel-forwarded-for` when platform-provided `VERCEL=1`; no fallback headers. IPv6 canonicalized; no raw IP logs. Local dev adapter without trusted ingress fails closed.

Changed: server/audit/interview-store.js, handler.js, interview.test.js; added interview-rate-limit.test.js; updated docs/AUDIT_BACKEND.md, README protection wording and this handoff. Frontend, prompts, result schema, existing WAF and Production configuration untouched. No paid calls, commit/push or M4 work.

Validation: relevant 21 tests PASS; full 51 tests PASS. New cases cover 1-18 allowed,19 blocked, independent IPs, fixed-window reset, concurrent handlers, no state/provider access on block, Redis failure and header spoofing rejection. Production build PASS (existing browser-data/Zod warnings); routing validation PASS; git diff --check PASS; secret-pattern scan PASS. .env.local remains ignored/untracked; values were not inspected. No actual Redis or paid provider calls were made.

Exact Preview next action: deploy/redeploy this patch with existing Preview Redis credentials, stable origin and enable flags; retain the current analyze WAF unchanged, no interview WAF rule. Run the documented 19-request invalid-input probe in a fresh window (400 for 1-18,429 for 19, no Astra invocation), verify reset after 600 seconds, then explicitly run the two synthetic M3 workflows. Actual Redis/Vercel enforcement and live interview validation remain PENDING. Production remains unchanged; M4 not started.
