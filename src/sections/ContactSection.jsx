  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setStatus({
        type: "success",
        message:
          "Thanks. Your request was sent successfully. We will respond within 2 business days.",
      });
      setForm(initialForm);
    } catch (error) {
      setStatus({
      ...
      <div className="section-container grid gap-12 lg:grid-cols-[1.1fr,0.9fr]">
        <div>
          ...
          <h2 className="mt-3 text-3xl font-semibold text-slate-900 dark:text-white md:text-4xl">
            {t("Request an automation consultation for your Revit workflow.")}
          </h2>
        ...
        >
            ...
            {[
              ...
              >
                <input
                  ...
                  onChange={(event) =>
                    updateField("intent", event.target.value)
                  }
                  className="sr-only"
                />
                {t(label)}
          ...
          {status && (
            <p
              className={`mt-4 text-center text-sm ${status.type === "error" ? "text-red-600 dark:text-red-300" : "text-brand-600 dark:text-brand-300"}`}
            >
              {t(status.message)}
            </p>
          )}
