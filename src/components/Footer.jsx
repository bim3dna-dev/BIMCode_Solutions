...
export default function Footer() {
  ...
  return (
    <footer className="border-t border-slate-200/70 bg-white/80 py-8 text-sm text-slate-500 transition dark:border-slate-800/70 dark:bg-slate-950/80 dark:text-slate-400">
      <div className="section-container flex flex-col gap-4 md:flex-row md:flex-wrap md:items-center md:justify-between md:gap-6">
        <a
          href="/#hero"
          className="flex items-center gap-3 transition hover:text-brand-600 dark:hover:text-brand-300"
        >
          <img
            src={logoPng}
        ...
        <div className="flex flex-col items-center gap-2 md:items-start">
          ...
          <a
            ...
            className="transition hover:text-[#4f6a7d] dark:hover:text-[#9bb6c9]"
          >
            {t("bimcodesolutions@gmail.com")}
          </a>
        </div>
        <p className="text-center md:text-left">
          &copy; {new Date().getFullYear()}{" "}
          {t("BIMCode Solutions. All rights reserved.")}
        </p>
      </div>
    </footer>
  );
